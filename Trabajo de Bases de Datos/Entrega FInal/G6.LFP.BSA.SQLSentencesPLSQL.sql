...
CREATE OR REPLACE TRIGGER trg_CalcRtCst 
BEFORE INSERT OR UPDATE ON Transport 
FOR EACH ROW 
BEGIN 
    -- Calcula el costo de la ruta basado en el valor de TRoute 
    :NEW.Route_Cost := :NEW.TRoute * 0.4; 
END; 
/

CREATE OR REPLACE TRIGGER trg_UpdateLogDate 
BEFORE UPDATE ON Users 
FOR EACH ROW 
BEGIN 
    -- Actualiza Log_Date con la fecha y hora actuales 
    :NEW.Log_Date := SYSDATE; 
END; 
/

CREATE OR REPLACE TRIGGER trg_CalculateAgeOnEmployees 
BEFORE INSERT OR UPDATE ON Employees 
FOR EACH ROW 
BEGIN 
    :NEW.Age := TRUNC(MONTHS_BETWEEN(SYSDATE, :NEW.Birth_Date) / 12) - 
                 CASE  
                     WHEN TO_CHAR(:NEW.Birth_Date, 'MMDD') > TO_CHAR(SYSDATE, 'MMDD') THEN 1  
                     ELSE 0  
                 END; 
END; 
/

CREATE OR REPLACE TRIGGER trg_UpdateAvgRating 
AFTER INSERT OR UPDATE OR DELETE ON RaitingsTransaction 
FOR EACH ROW 
BEGIN 
    -- Actualizar la calificación promedio después de INSERT o DELETE 
    IF INSERTING OR UPDATING THEN 
        UPDATE Users 
        SET Avg_Raitings = ( 
            SELECT NVL(AVG(rt.Rating), 0)  -- Calcula el promedio, usa 0 si no hay registros 
            FROM RaitingsTransaction rt 
            WHERE rt.UserR_Sell_ID = :NEW.UserR_Sell_ID 
        ) 
        WHERE UserID = :NEW.UserR_Sell_ID; 
    ELSIF DELETING THEN 
        UPDATE Users 
        SET Avg_Raitings = ( 
            SELECT NVL(AVG(rt.Rating), 0)  -- Calcula el promedio, usa 0 si no hay registros 
            FROM RaitingsTransaction rt 
            WHERE rt.UserR_Sell_ID = :OLD.UserR_Sell_ID 
        ) 
        WHERE UserID = :OLD.UserR_Sell_ID; 
    END IF; 
END; 
/

CREATE OR REPLACE TRIGGER trg_CalculateDwellTime 
AFTER INSERT OR UPDATE ON Employees 
FOR EACH ROW 
BEGIN 
    -- Verificar si el contrato existe para calcular Dwell_Time 
    IF :NEW.ContractID IS NOT NULL THEN 
        UPDATE Employees e 
        SET Dwell_Time = TRUNC(MONTHS_BETWEEN(SYSDATE, (SELECT c.Sign_Date FROM Contracts c WHERE c.ContractID = :NEW.ContractID)) / 12) 
        WHERE e.Emp_Num = :NEW.Emp_Num 
        AND Dwell_Time != TRUNC(MONTHS_BETWEEN(SYSDATE, (SELECT c.Sign_Date FROM Contracts c WHERE c.ContractID = :NEW.ContractID)) / 12); 
    END IF; 
END; 
/

CREATE OR REPLACE TRIGGER trg_update_autonomy 
BEFORE INSERT OR UPDATE ON Automoviles 
FOR EACH ROW 
BEGIN 
    -- Calcula la autonomía si Fuel_Capacity y Consume tienen valores válidos 
    IF :NEW.Fuel_Capacity IS NOT NULL AND :NEW.Consume IS NOT NULL AND :NEW.Consume > 0 THEN 
        :NEW.Autonomy := (:NEW.Fuel_Capacity / :NEW.Consume) * 100; 
    ELSE 
        :NEW.Autonomy := NULL; -- Define como NULL si los datos no son válidos 
    END IF; 
END; 
/

CREATE OR REPLACE TRIGGER trg_UpdateWorWarInventory 
AFTER INSERT OR UPDATE ON WorWar 
FOR EACH ROW 
BEGIN 
    -- Verificar que exista un WareZipCode asociado y actualizar el inventario 
    UPDATE WorWar 
    SET Inventory = ( 
        SELECT NVL(wh.Inv_Park_Slots, 0) + NVL(wh.Inv_Pieces_Num, 0) 
        FROM WareHouses wh 
        WHERE wh.WareZipCode = :NEW.WareZipCode 
    ) 
    WHERE WorWarID = :NEW.WorWarID; 
END; 
/
